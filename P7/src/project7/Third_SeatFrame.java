...
public class Third_SeatFrame extends JFrame {
	JPanel panel,seat;
	 ...
	 */
	public Third_SeatFrame(final String movieName,int people,final int price,final int ticketNum,final String time,final JFrame parent) {
		setSize(500,400);
		setTitle("좌석 선택");
		member = people;
		...
		prev.addActionListener(new ActionListener() {									// prev 버튼 이벤트 리스너
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				Third_SeatFrame.this.dispose();												// 좌석 선택 화면은 dispose
			}
		});
		next.addActionListener(new ActionListener() {									// next 버튼 이벤트 리스너
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(member>0) 
					...
					new MsgBox(new JFrame("") ,"영화 이름 : "+movieName,"예약 번호 : "+ticketNum, etc,"좌     석   : "+seats, false,Third_SeatFrame.this); // 가공한 정보를 msgbox로 띄움
					msg +=seats+"\t"+time;
