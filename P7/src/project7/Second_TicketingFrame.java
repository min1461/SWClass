...
public class Second_TicketingFrame extends JFrame implements ItemListener, ActionListener{
	...
	public Second_TicketingFrame(){
		...
		home.addActionListener(new ActionListener() { 					// home 버튼 이벤트 리스너 이전 frame의 home 이벤트 리스너와 같음
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				Second_TicketingFrame.this.dispose();
				new First_MainFrame().show();
			}
		});
	...
	public void actionPerformed(ActionEvent e){
		...
		// 버튼에 적힌 문자열을 읽어온다.
			...
			else
			{
				...
				Second_TicketingFrame.this.setVisible(false); // 현재 티켓고르는 화면은 visivle false로 해줌 dispose 안한것은 seatframe에서 prev버튼 처리를 쉽게하기위함임 dispose해주면 사용자가 입력한 모든 정보가 날라가기 때문
				/*
				 * 지금 선택한 영화의 모든 정보를 seatFrame에 넘겨줌 seatFrame에선 모든 좌석을 선택하면 영화 예매가 완료됨
				 */
				Third_SeatFrame seat = new Third_SeatFrame(movieName.getItem(select),l_nadult_ticket+l_nstudent_ticket,l_totalPrice,ticketNum,l_strmovie_time,Second_TicketingFrame.this);
