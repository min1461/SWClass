...
public class Fourth_CheckTicketFrame extends JFrame{
	ArrayList<String> ticket; 											// text파일에서 예매된 정보를 가져와서 arraylist에 넣어줌
	JPanel panel;
	JLabel label,numLabel;
	JButton next,home;
	JTextField numText;
	public Fourth_CheckTicketFrame(){
		setSize(500,400);
		setTitle("CGV");
		...
		home.addActionListener(new ActionListener() { 					// home 버튼 이벤트 리스너
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				Fourth_CheckTicketFrame.this.dispose(); 						// 현재 frame을 dispose 해줌
				new First_MainFrame().show(); 								// mainframe을 show 해줌
			}
		});
